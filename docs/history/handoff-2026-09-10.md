...
## Current checkpoint: adoption audit completed, 10 September 2026

Report: [Product UI adoption audit](docs/audits/2026-09-10-adoption-audit.md). Evidence and independent reproduction templates are linked from the report. Audited library `main` at `78dd64e16d2aa7ae7e106687e11cb18bcc46ea04`, 22 commits ahead of tagged v0.9.0. This task changed audit documentation and this handoff only; production source, dependencies, accepted snapshots and ROADMAP.md remain unchanged. Nothing was committed, merged, tagged, published or deployed.

**Readiness is conditional.** The real release archive and packed main build in independent Next.js 15/React 19/Tailwind 4 fixtures; the prior Tailwind 3 CSS failure reproduces on 0.6 and passes on main. Fresh tests: 30 unit tests, 219 light stories and 219 dark stories, plus types/lint/tokens/API/CSS/build checks. Existing CI on the exact main commit is green. Independent browser probes nevertheless found failed-save recovery, stale column selection, locale parsing, loading/form combobox semantics, date-panel clipping, multiple sticky-column overlap and accessibility gaps. Prioritise the report's ordered backlog before widening adoption or starting more feature work. The changelog/version gap remains a P1 release blocker.

**Consumer evidence must stay precise.** `cd_capacity` was inspected strictly read-only at `6e6691d5902032233154d0c1dfd4e1daf178f282`. Its 0.6.0 Dashboard adoption is partial; no Product UI DashboardSidebar import exists at that revision. The actual AppSidebar remains local. Its latest existing CI fails at npm ci because the npm lockfile is stale against the manifest; the pnpm lock describes a different resolved stack. No install, build, test, generator, development server or package script ran in the consumer checkout. Its 484 tracked paths have identical before/after content hashes and final status is clean. No local or remote consumer modifications or coworker messages were made.

Earlier statements below about successful consumer builds and “read-only clones” are historical claims and were **not** independently reproduced in this audit. A passing independent fixture is not a successful consumer upgrade. The registry hosting retirement remains in force; the report proposes clarifying stale install guidance and the still-active publishing workflow, not reviving Pages.

The report explicitly marks incomplete coverage: no live/authenticated consumer screen verification, manual screen-reader session, Safari/Firefox or real forced-colour/touch-device validation; no complete timezone/async-race/large-data performance or legal/licence audit. Keep the authoritative roadmap unchanged until proposed status corrections and scope decisions are reviewed.

## Prior checkpoint and historical work log
