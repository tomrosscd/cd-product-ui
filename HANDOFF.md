...
Last updated: 10 September 2026 (post-0.9.0, four PRs open/merged this round). Update this file at each meaningful checkpoint and before stopping or handing off. Read AGENTS.md and the linked release documentation before editing.
...
**PR status this round:**

1. **[PR #22](https://github.com/tomrosscd/cd-product-ui/pull/22)** (`feature/sidebar-nested-sections`) — merged. `DashboardSidebar`'s `SidebarSection.items` now accepts `SidebarEntry[]` (sections can nest). Also fixed three real bugs found via live visual QA: brand-row toggle overflow, excessive collapsed-rail spacing, dropdown-ring corner-clipping (switched to inset `box-shadow`). `--cui-focus-width` 2px → 1px.
2. **[PR #23](https://github.com/tomrosscd/cd-product-ui/pull/23)** (`feature/button-variants-sizes`) — merged. `Button` gained `outline`/`destructive`/`link` variants and `sm`/`lg`/`icon` sizes. Confirmed via the consumer's own `SheetsImportPanel.tsx`, which uses `variant="outline"` on a local shadcn Button because this library never had one.
3. **[PR #25](https://github.com/tomrosscd/cd-product-ui/pull/25)** (`feature/stepper`) — **open, CI-green, blocked on merge.** `gh pr merge` was denied by the Claude Code auto-mode permission classifier; needs the user to merge it directly (or grant a permission rule) — do not attempt to work around that denial. `Stepper` primitive, justified by the consumer's own `WizardStepIndicator.tsx` (ROADMAP.md #16). Caught a real `scrollable-region-focusable` a11y violation via the Storybook a11y addon, fixed with `tabIndex={0}` (same pattern already used by `Table`).
4. **[PR #26](https://github.com/tomrosscd/cd-product-ui/pull/26)** (`feature/notification-centre`) — open, CI-green, awaiting review/merge. `NotificationCentre` primitive (bell trigger + Radix Popover panel), justified by the consumer's own `NotificationBell.tsx` (ROADMAP.md #27). See "Notification centre: a real Vitest browser-mode crash" below — worth reading before touching any story that clicks a real `<a href>`.

**Two follow-up fixes needed after first CI run, worth knowing about for next time (applies to any PR that adds a `.cui-*` class or touches tokens):**
