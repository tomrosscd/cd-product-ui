...
**Full release readiness confirmed locally** (all after the version bump to 0.12.0): `pnpm test:dark` (250/250), `pnpm check` (type-check, lint, `pnpm test` 310/310, build, CSS check, Storybook build), `pnpm package:check` (328 packed files, packed browser layout checks in light/dark at 1440/390px), `pnpm next:check` (Next.js 16), `pnpm compatibility:check` (Next.js 15/npm/Tailwind 4 + Tailwind 3), `pnpm docs:check`, `pnpm registry:check`, `pnpm test:coverage` (93.4% statements), `pnpm release:check`. `docs/component-catalogue.md` needed a `FormSection` entry (docs:check caught the missing export reference) and `docs/release-0.12.md` was added. Remaining step: push the branch, open the PR, wait for green CI, merge, tag `v0.12.0`, verify the publish workflow.
