...
2. ~~**Numeric and financial cell treatment.**~~ **Done.** `meta: { numeric }` gives end alignment and tabular figures, `meta: { sticky }` pins a leading identifier column, and a column definition's `footer` renders a totals row. The stories render missing values as missing and exclude them from totals rather than counting them as zero, per the metric rules in AGENTS.md.
