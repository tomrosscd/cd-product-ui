...
1. ~~**Apply TanStack column sizes to rendered cells.**~~ **Done.** Declaring any `size` switches the table to fixed layout; sizes act as ratios when the table fills its container. The library already depends on `@tanstack/react-table`, and consumers can already set `size` on a column definition, but nothing reads it, so column widths are ignored today. Start here: it is the smallest change with the widest effect, and it unblocks the pinned-column and financial-table work.
2. **Numeric and financial cell treatment.** _Partly done:_ `meta: { numeric }` gives end alignment and tabular figures, and the story shows missing rendered as missing rather than zero. Still open: sticky identifier columns and summary footers. Right-aligned numerals, tabular figures, consistent hours and currency formatting, explicit negative and missing-value rendering. The retainers screen needs all of it, and "missing" must stay visually distinct from zero, per the metric rules in AGENTS.md.
...
- ~~**Softer negative badge.**~~ **Done.** `.cui-badge-negative` used `--cui-surface-page`, within 1.00:1 of the page behind it, so it read as bare text beside the tinted positive and warning badges. Now uses `--cui-surface-negative`, measured at 1.20:1 against the page, between warning's 1.17 and positive's 1.37, with 5.16:1 for the text on it. This changes an existing default appearance, so it is called out in the changelog.
